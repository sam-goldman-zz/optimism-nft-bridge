...
describe('Lib_SecureMerkleTrie', () => {
            ...
            describe(`inside a trie with ${nodeCount} nodes`, () => {
                ...
                for (let i = 0; i < nodeCount; i += nodeCount / (nodeCount > 8 ? 8 : 1)) {
                    it(`should correctly prove inclusion for node #${i}`, async () => {
                        ...
                        (0, setup_1.expect)(await Lib_SecureMerkleTrie.verifyInclusionProof(test.key, test.val, test.proof, test.root)).to.equal(true);
                    });
                }
            });
        }
            ...
            describe(`inside a trie with ${nodeCount} nodes`, () => {
                ...
                for (let i = 0; i < nodeCount; i += nodeCount / (nodeCount > 8 ? 8 : 1)) {
                    it(`should correctly update node #${i}`, async () => {
                        ...
                        (0, setup_1.expect)(await Lib_SecureMerkleTrie.update(test.key, test.val, test.proof, test.root)).to.equal(test.newRoot);
                    });
                }
            });
        }
            ...
            describe(`inside a trie with ${nodeCount} nodes`, () => {
                ...
                for (let i = 0; i < nodeCount; i += nodeCount / (nodeCount > 8 ? 8 : 1)) {
                    it(`should correctly get the value of node #${i}`, async () => {
                        const test = await generator.makeInclusionProofTest(i);
                        (0, setup_1.expect)(await Lib_SecureMerkleTrie.get(test.key, test.proof, test.root)).to.deep.equal([true, test.val]);
                    });
                }
            });
        }
    ...
    describe('getSingleNodeRootHash', () => {
        ...
        it(`should get the root hash of a trie with a single node`, async () => {
            ...
            (0, setup_1.expect)(await Lib_SecureMerkleTrie.getSingleNodeRootHash(test.key, test.val)).to.equal(test.root);
        });
    });
});
